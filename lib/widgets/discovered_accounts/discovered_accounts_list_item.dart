...
class DiscoveredAccountsListItem extends StatefulWidget {
  ...
  final Function(DiscoveredAccountsListItemState listItemStatePressed)
      onLinkAccountToBUPressed;

  const DiscoveredAccountsListItem(
      {Key? key,
      required this.dataItem,
      required this.onLinkAccountToBUPressed})
      : super(key: key);
  // @override
...
class DiscoveredAccountsListItemState
    extends State<DiscoveredAccountsListItem> {
  late DiscoveredAccountsListDataItem _dataItem;
  @override
  initState() {
    print('init state called for DiscoveredAccountsListItemState');
    super.initState();
  ...
  setIsLoading(bool isLoading) {
    ...
    setState(() {
      print(
          'setIsLoading called: $isLoading for ${widget.dataItem.displayText}');
      _dataItem = _dataItem;
    });
  }
  ...
  Widget build(BuildContext context) {
    final displayText =
        _dataItem.isLoading ? '...backend call' : widget.dataItem.displayText;
    print(
        'build called for ${widget.dataItem.displayText} isLoading: ${_dataItem.isLoading} state. state _dataItem.displayText: ${_dataItem.displayText}');
    //hack
    // _setDataItemIfNeeded();
